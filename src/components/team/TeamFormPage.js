...
const TeamFormPage = ({ mode }) => {
  ...
  const pageTitle = useMemo(() => (isEditMode ? "Edit Team Member" : "Create Team Member"), [isEditMode]);
  ...
  useEffect(() => {
    ...
    const fetchMember = async () => {
      ...
      try {
        ...
        if (!item) {
          showErrorToast("Team member not found");
          navigate("/team");
          return;
        }
        setForm({
        ...
        showErrorToast(err.response?.data?.message || "Failed to load team member");
        navigate("/team");
      } finally {
        setIsPageLoading(false);
      }
  ...
  const validateForm = () => {
    ...
    if (!form.subtitle.trim()) nextErrors.subtitle = "Sub title is required.";
    ...
    if (!form.category_id) nextErrors.category_id = "Team category is required.";
  ...
  const handleSave = async (event) => {
    ...
    try {
      ...
      if (isEditMode) {
        await apiClient.put(`/team/${id}`, payload, {
      ...
      showErrorToast(err.response?.data?.message || "Unable to save team member");
    } finally {
      setIsSaving(false);
    }
  };
