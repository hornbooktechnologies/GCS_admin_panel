...
const NodelOfficerFormPage = ({ mode }) => {
  ...
  return (
    <div className="max-w-[1600px] space-y-6">
      <div>
        <Button type="button" variant="ghost" className="-ml-3 mb-2 rounded-lg px-3 text-slate-500" onClick={() => navigate("/nodel-officers")}>
      ...
      <form onSubmit={handleSave} className="rounded-lg border border-white/60 bg-white/80 p-6 shadow-sm backdrop-blur-xl">
        <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <textarea value={form.address} onChange={(event) => handleInputChange("address", event.target.value)} rows={5} className={`min-h-[140px] w-full rounded-lg border bg-white px-4 py-3 text-sm text-slate-700 outline-none ${formErrors.address ? "border-red-300 focus:ring-2 focus:ring-red-500" : "border-slate-200 focus:ring-2 focus:ring-primary/30"}`} />
              <FieldError>{formErrors.address}</FieldError>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Phone Number</label>
                <Input value={form.phone_number} onChange={(event) => handleInputChange("phone_number", event.target.value)} className={`rounded-lg ${formErrors.phone_number ? "border-red-300 focus-visible:ring-red-500" : ""}`} />
                <FieldError>{formErrors.phone_number}</FieldError>
              </div>
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Email ID</label>
                <Input type="email" value={form.email} onChange={(event) => handleInputChange("email", event.target.value)} className={`rounded-lg ${formErrors.email ? "border-red-300 focus-visible:ring-red-500" : ""}`} />
        ...
        <div className="mt-6 flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
          <Button type="button" variant="outline" className="rounded-lg" onClick={() => navigate("/nodel-officers")}>
            Cancel
          </Button>
