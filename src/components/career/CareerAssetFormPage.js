...
const CareerAssetFormPage = ({ config, mode }) => {
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      ...
      try {
        const response = await apiClient.get(`${config.apiBase}/${id}`);
        const item = response.data?.data;
        if (!item) {
          showErrorToast(`${config.singular} not found`);
        ...
        showErrorToast(err.response?.data?.message || `Failed to load ${config.singular.toLowerCase()}`);
        navigate(config.listPath);
      } finally {
        setIsPageLoading(false);
      }
    ...
    fetchItem();
  }, [config.apiBase, config.listPath, config.singular, id, isAdmin, isEditMode, navigate, showErrorToast]);
  ...
  const handleSave = async (event) => {
    ...
    try {
      ...
      if (isEditMode) {
        await apiClient.put(`${config.apiBase}/${id}`, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        showSuccessToast(`${config.singular} updated successfully`);
      } else {
        await apiClient.post(config.apiBase, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        showSuccessToast(`${config.singular} created successfully`);
      }

      navigate(config.listPath);
    } catch (err) {
      showErrorToast(err.response?.data?.message || `Unable to save ${config.singular.toLowerCase()}`);
    } finally {
      setIsSaving(false);
    }
  };
    ...
    return (
      <div className="rounded-lg border border-white/60 bg-white/80 p-8 shadow-sm">
        <div className="flex items-center gap-3 text-sm font-medium text-slate-500">
          ...
          Loading {config.singular.toLowerCase()} editor...
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1600px] space-y-6">
      <div>
        <Button type="button" variant="ghost" className="-ml-3 mb-2 rounded-lg px-3 text-slate-500" onClick={() => navigate(config.listPath)}>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <div role="button" tabIndex={0} onClick={() => pdfInputRef.current?.click()} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); pdfInputRef.current?.click(); } }} className={`cursor-pointer rounded-lg border border-dashed p-6 text-center transition-all ${formErrors.pdf ? "border-red-300 bg-red-50/40" : "border-slate-200 bg-slate-50 hover:border-primary/40 hover:bg-white"}`}>
                <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-lg bg-white text-primary shadow-sm"><Upload className="h-6 w-6" /></div>
            ...
            <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-slate-500">
              {existingPdfUrl ? <a href={existingPdfUrl} target="_blank" rel="noreferrer" className="inline-flex items-center text-sm font-semibold text-primary"><FileText className="mr-2 h-4 w-4" />Open current PDF</a> : <FileText className="mx-auto h-8 w-8" />}
            </div>
          </div>
        </div>

        <div className="mt-6 flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
          <Button type="button" variant="outline" className="rounded-lg" onClick={() => navigate(config.listPath)}>Cancel</Button>
          <Button type="submit" className="rounded-lg" disabled={isSaving}>
            {isSaving ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}
            {isSaving ? "Saving..." : isEditMode ? `Update ${config.singular}` : `Create ${config.singular}`}
          </Button>
        </div>
      </form>
    </div>
...
export default CareerAssetFormPage;
