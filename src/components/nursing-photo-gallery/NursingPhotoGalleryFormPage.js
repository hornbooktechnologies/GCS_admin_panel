...
const NursingPhotoGalleryFormPage = ({ mode }) => {
  ...
  const pageTitle = useMemo(() => (isEditMode ? "Edit Nursing Photo" : "Create Nursing Photo"), [isEditMode]);
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      ...
      try {
        const response = await apiClient.get(`/nursing-photo-gallery/${id}`);
        const item = response.data?.data;
        if (!item) {
          showErrorToast("Nursing photo not found");
          navigate("/nursing-photo-gallery");
          return;
        }
        ...
        showErrorToast(err.response?.data?.message || "Failed to load nursing photo");
        navigate("/nursing-photo-gallery");
      } finally {
        setIsPageLoading(false);
      }
  ...
  const handleSave = async (event) => {
    ...
    try {
      ...
      if (isEditMode) {
        await apiClient.put(`/nursing-photo-gallery/${id}`, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        showSuccessToast("Nursing photo updated successfully");
      } else {
        await apiClient.post("/nursing-photo-gallery", payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        showSuccessToast("Nursing photo created successfully");
      }

      navigate("/nursing-photo-gallery");
    } catch (err) {
      showErrorToast(err.response?.data?.message || "Unable to save nursing photo");
    } finally {
      setIsSaving(false);
    }
  };
...
export default NursingPhotoGalleryFormPage;
