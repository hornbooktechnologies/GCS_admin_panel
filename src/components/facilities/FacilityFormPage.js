...
const FacilityFormPage = ({ mode }) => {
  ...
  const pageTitle = useMemo(() => (isEditMode ? "Edit Facility" : "Create Facility"), [isEditMode]);
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      ...
      try {
        ...
        if (!item) {
          showErrorToast("Facility not found");
          navigate("/facilities");
          return;
        }
        ...
        showErrorToast(err.response?.data?.message || "Failed to load facility");
        navigate("/facilities");
      } finally {
        setIsPageLoading(false);
      }
  ...
  const handleSave = async (event) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        showSuccessToast("Facility created successfully");
      }

      navigate("/facilities");
    } catch (err) {
      showErrorToast(err.response?.data?.message || "Unable to save facility");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-[1600px] space-y-6">
      <div>
        <Button type="button" variant="ghost" className="-ml-3 mb-2 rounded-lg px-3 text-slate-500" onClick={() => navigate("/facilities")}>
      ...
      <form onSubmit={handleSave} className="rounded-lg border border-white/60 bg-white/80 p-6 shadow-sm backdrop-blur-xl">
        <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="space-y-2">
            ...
            >
              {imagePreviewUrl ? (
                <img src={imagePreviewUrl} alt="Preview" className="mx-auto mb-4 aspect-[16/11] w-full rounded-lg object-cover" />
              ) : (
                <div className="mx-auto mb-4 flex aspect-[16/11] w-full items-center justify-center rounded-lg bg-white text-slate-400 shadow-sm">
            ...
            <ImageUploadGuidance requirementKey="facility" file={form.image} src={imagePreviewUrl} />
          </div>
        </div>

        <div className="mt-6 flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
          <Button type="button" variant="outline" className="rounded-lg" onClick={() => navigate("/facilities")}>
            Cancel
          </Button>
          <Button type="submit" className="rounded-lg" disabled={isSaving}>
            {isSaving ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}
            {isSaving ? "Saving..." : isEditMode ? "Update Facility" : "Create Facility"}
          </Button>
        </div>
      </form>
    </div>
