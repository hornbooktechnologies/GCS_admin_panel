...
const CareerInternshipPositions = () => {
  ...
  const fetchItems = async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get("/career/internship-positions");
      setItems(response.data?.data?.items || []);
    } catch (err) {
      showErrorToast(err.response?.data?.message || "Failed to load internship positions");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (item) => {
    ...
    if (!confirmed) return;
    try {
      await apiClient.delete(`/career/internship-positions/${item.id}`);
      showSuccessToast("Internship position deleted successfully");
      fetchItems();
    } catch (err) {
      showErrorToast(err.response?.data?.message || "Unable to delete internship position");
    }
  };
  ...
  return (
    <div className="max-w-[1600px] space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">Internship Positions</h1>
          <p className="mt-1 text-sm font-medium text-slate-500">Manage title, image, and PDF for internship positions.</p>
        </div>
        <div className="flex gap-3">
          ...
          <Button type="button" className="rounded-xl" onClick={() => navigate("/career/internship-positions/new")}>
      ...
      <div className="overflow-hidden rounded-3xl border border-white/60 bg-white/80 shadow-sm backdrop-blur-xl">
        {isLoading ? (
          <div className="py-12 text-center text-sm font-medium text-slate-500">Loading internship positions...</div>
        ) : items.length === 0 ? (
          <div className="py-12 text-center text-sm text-slate-500">No internship positions found.</div>
        ) : (
                  ...
                  <tr key={item.id}>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-2">
                        <Button type="button" variant="outline" className="rounded-xl" onClick={() => navigate(`/career/internship-positions/${item.id}/edit`)}>
...
export default CareerInternshipPositions;
