...
const Sympotms = () => {
  ...
  const canCreate = hasPermission(user, "symptoms", "create");
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      setItems(response.data?.data?.sympotms || []);
    } catch (err) {
      showErrorToast(err.response?.data?.message || "Failed to load symptoms");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (item) => {
    ...
    try {
      ...
      showErrorToast(err.response?.data?.message || "Unable to delete symptom");
    }
  };
  ...
  return (
    <div className="max-w-[1600px] space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">Symptoms</h1>
      ...
      <div className="overflow-hidden rounded-3xl border border-white/60 bg-white/80 shadow-sm backdrop-blur-xl">
        {isLoading ? (
          ...
          <div className="py-12 text-center text-sm text-slate-500">No symptoms found.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-100">
              <thead className="bg-slate-50/80">
                <tr>
                  <th className="px-5 py-4 text-left text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Symptom</th>
                  <th className="px-5 py-4 text-left text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Subtitle</th>
                  <th className="px-5 py-4 text-left text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Potential Causes</th>
                ...
                {items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-5 py-4 text-sm text-slate-600">
                      <div className="line-clamp-3 max-w-xl">{item.subtitle}</div>
                    </td>
